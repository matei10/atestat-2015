program lgoritmul_roy_floyd_maxim;
uses crt;
...
for i := 1 to n do
    for j := 1 to n do
        if i <> j then { un drum de la un nod la el insusi nu are sens }
            if (costuri[i, j] > m_infinit) then { daca exista drum de la i -> j }
                begin
...
for i := 1 to n do { pentru fiecare nod in graf }
    for j := 1 to n do { pentru pereche de noduri i, j }
        for k := 1 to n do { verificam pentru fiecare nod in graf }
            if (k <> i) AND ( k <> j) AND ( costuri[i, k] > m_infinit) AND ( costuri[k, j] > m_infinit) then { daca nodul k este diferit de nodurile i si j, si daca nu avem drumuri inexistente, adica costuri = m_infinit  }
